...
function cliPath(): string {
  const candidates = [
    join(dirname(process.execPath), 'resources', 'app', 'lib', 'backend', 'resources', 'arduino-cli.exe'),
    'D:\\编程\\Arduino IDE\\resources\\app\\lib\\backend\\resources\\arduino-cli.exe'
  ];
  return candidates.find(existsSync) || 'arduino-cli';
}

function runCli(args: string[], cwd?: string): Promise<CliResult> {
  return new Promise(resolve => {
    execFile(cliPath(), args, { timeout: 30000, cwd }, (error, stdout, stderr) => {
      const exitCode = error ? Number((error as NodeJS.ErrnoException & { code?: number }).code) || 1 : 0;
      resolve({ code: exitCode, stdout, stderr });
    });
  });
}

type FixKind = 'semicolon' | 'library';
type Diagnosis = { title: string; advice: string; file?: string; line?: number; column?: number; fixKind?: FixKind; header?: string; library?: string; correctedHeader?: string };
type ChangeRecord = { time: string; file: string; reason: string; before: string; after: string };
type ProjectBrief = { goal: string; sensor: string; output: string; sensorPin: string; outputPin: string; wiringConfirmed: boolean };

function diagnosis(text: string): Diagnosis {
...
function currentSketch(): { folder: string; file: string } | undefined {
  const editor = plugin.window.activeTextEditor;
  const file = editor?.document.uri.fsPath;
  if (!file || !file.toLowerCase().endsWith('.ino')) return undefined;
  return { folder: dirname(file), file };
}

function compileArgs(folder: string): string[] {
  return ['compile', '--fqbn', 'arduino:avr:uno', '--libraries', join(homedir(), 'Documents', 'Arduino', 'libraries'), folder];
}

function projectCode(brief: ProjectBrief): string {
...
function wiringChecklist(brief: ProjectBrief): string[] {
  return [
...
async function revealError(d: Diagnosis): Promise<void> {
...
async function fixLibrary(d: Diagnosis, context: plugin.ExtensionContext, refresh: () => void): Promise<void> {
  if (!d.library) { plugin.window.showWarningMessage('暂时无法确定对应库，请打开 Arduino IDE 的库管理器搜索头文件名称。'); return; }
  plugin.window.showInformationMessage(`正在安装 ${d.library}…`);
  const installed = await runCli(['lib', 'install', d.library]);
  if (installed.code !== 0) { plugin.window.showErrorMessage(`安装失败：${installed.stderr || installed.stdout}`); return; }
  if (d.file && d.header && d.correctedHeader && d.header !== d.correctedHeader && existsSync(d.file)) {
    const doc = await plugin.workspace.openTextDocument(d.file);
    ...
    await plugin.workspace.applyEdit(edit); await doc.save();
...
async function showDiagnosis(log: string, context: plugin.ExtensionContext, refresh: () => void): Promise<void> {
...
function scanSummary(stdout: string): { env: string; advice: string } {
  try {
    const data = JSON.parse(stdout) as { detected_ports?: Array<{ port?: { address?: string; protocol_label?: string; properties?: { vid?: string; pid?: string } }, matching_boards?: Array<{ name?: string; fqbn?: string }> }> };
    const usb = (data.detected_ports || []).filter(item => item.port?.properties?.vid);
    if (!usb.length) return { env: '未发现 USB 串口', advice: '尚未检测到 USB 主板。请检查数据线、驱动和接口。' };
...
export function start(context: plugin.ExtensionContext): void {
  ...
  class AgentProvider implements plugin.TreeDataProvider<AgentItem> {
    private readonly changed = new plugin.EventEmitter<AgentItem | undefined>();
    readonly onDidChangeTreeData = this.changed.event;
    refresh(): void { this.changed.fire(undefined); }
    getTreeItem(item: AgentItem): plugin.TreeItem { return item; }
    getChildren(element?: AgentItem): AgentItem[] {
      if (element) return element.children;
      const records = context.workspaceState.get<ChangeRecord[]>('changeRecords', []);
      const brief = context.workspaceState.get<ProjectBrief>('projectBrief');
  ...
  context.subscriptions.push(plugin.commands.registerCommand('arduinoFirstRunAgent.createProject', async () => {
    ...
    if (!outputPin) return;
    const brief: ProjectBrief = { goal, sensor, output, sensorPin, outputPin, wiringConfirmed: false };
    const uri = await plugin.window.showSaveDialog({ title: '保存 Agent 生成的 Arduino 工程', defaultUri: plugin.Uri.file(join(homedir(), 'Documents', 'Arduino', 'agent_project', 'agent_project.ino')), filters: { 'Arduino Sketch': ['ino'] } });
    if (!uri) return;
    ...
    if (selected === '开始核对接线') await plugin.commands.executeCommand('arduinoFirstRunAgent.confirmWiring');
  }));
  context.subscriptions.push(plugin.commands.registerCommand('arduinoFirstRunAgent.confirmWiring', async () => {
    const brief = context.workspaceState.get<ProjectBrief>('projectBrief');
    if (!brief) { plugin.window.showWarningMessage('请先创建项目方案。'); return; }
    ...
    if (!selected) return;
    if (selected.length !== checks.length) { plugin.window.showWarningMessage(`还有 ${checks.length - selected.length} 项接线未确认。Agent 暂不把电路标记为正确。`); return; }
    ...
    if (next === '检查环境') await plugin.commands.executeCommand('arduinoFirstRunAgent.scan');
  }));
  context.subscriptions.push(plugin.commands.registerCommand('arduinoFirstRunAgent.scan', async () => {
    ...
    if (r.code !== 0) { plugin.window.showErrorMessage('Arduino CLI 未能启动'); return; }
  ...
  context.subscriptions.push(plugin.commands.registerCommand('arduinoFirstRunAgent.compile', async () => {
    const brief = context.workspaceState.get<ProjectBrief>('projectBrief');
    if (brief && !brief.wiringConfirmed) { plugin.window.showWarningMessage('请先完成接线确认，再进入代码检查。'); return; }
    const sketch = currentSketch();
    if (!sketch) { plugin.window.showWarningMessage('请先打开要检查的 .ino 文件。'); return; }
  ...
  context.subscriptions.push(plugin.commands.registerCommand('arduinoFirstRunAgent.upload', async () => {
    const brief = context.workspaceState.get<ProjectBrief>('projectBrief');
    if (brief && !brief.wiringConfirmed) { plugin.window.showWarningMessage('接线尚未确认。为避免错误接线带来的风险，暂不上传。'); return; }
  ...
  context.subscriptions.push(plugin.commands.registerCommand('arduinoFirstRunAgent.behavior', async (initial?: string) => {
    ...
    if (selected === '打开当前代码') {
      const sketch = currentSketch();
      if (sketch) await plugin.window.showTextDocument(await plugin.workspace.openTextDocument(sketch.file));
    }
    if (selected === '重新编译') await plugin.commands.executeCommand('arduinoFirstRunAgent.compile');
  }));
  ...
  const connect = (webview: plugin.Webview): void => {
    ...
    webview.onDidReceiveMessage(async message => {
      if (message.type === 'scan') {
        ...
        const sketch = currentSketch();
        if (!sketch) { webview.postMessage({ type: 'result', text: '请先打开要检查的 .ino 文件。' }); return; }
        const r = await runCli(compileArgs(sketch.folder), sketch.folder);
...
export function activate(context: plugin.ExtensionContext): void {
  start(context);
}
